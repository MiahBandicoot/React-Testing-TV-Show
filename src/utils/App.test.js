...
import {findByTestId, getByRole, getByText, render, screen} from '@testing-library/react'
import App from '../App'
...
test("App fetches data/renders data", async () => {
    
    fetchShow.mockResolvedValueOnce(showData)
    render(<App/>)
    const button = await screen.findByText('Select a season')
    userEvent.click(button);

    const season = await screen.findByText('Season 1')
